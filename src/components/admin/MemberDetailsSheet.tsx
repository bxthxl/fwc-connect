...
import { User, Phone, Mail, MapPin, Calendar, Users, KeyRound, Trash2, Loader2, Building2, Lock, Pencil, Send } from 'lucide-react';
...
export function MemberDetailsSheet({
  ...
  const handleSendMagicLink = async () => {
    ...
    try {
      ...
      const result = await resp.json();
      if (!resp.ok) throw new Error(result.error);
      toast({ title: 'Magic link sent', description: `A login link has been sent to ${member.email}. They will be prompted to set a new password.` });
    } catch (error: any) {
  ...
  return (
    <Sheet open={open} onOpenChange={(o) => { onOpenChange(o); if (!o) setIsEditing(false); }}>
      <SheetContent className="sm:max-w-md overflow-y-auto">
        <SheetHeader>
          ...
          <SheetDescription>{isEditing ? 'Edit member information' : 'Member details and role management'}</SheetDescription>
        </SheetHeader>
          ...
          {isEditing ? (
            ...
            <>
              ...
              <div className="border-t pt-4 space-y-2">
                ...
                {/* Set New Password */}
                <AlertDialog open={showSetPasswordDialog} onOpenChange={(o) => { setShowSetPasswordDialog(o); if (!o) setNewPassword(''); }}>
                ...
                {/* Send Magic Link */}
                <Button variant="outline" className="w-full justify-start" onClick={handleSendMagicLink} disabled={isSendingLink}>
                  {isSendingLink ? <Loader2 className="h-4 w-4 mr-2 animate-spin" /> : <Send className="h-4 w-4 mr-2" />}
